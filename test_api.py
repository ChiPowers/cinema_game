...
PICKLE_PATH = "data/professional.pkl"
...
def test_valid_chain(chain, start_actor, end_actor, start_id, end_id):
...
if __name__ == "__main__":
    ...
    game = game_resp.json()
    print("Start Actor:", game["start_name"], "| ID:", game["start_id"])
    print("End Actor:", game["end_name"], "| ID:", game["end_id"])
    ...
    if chain:
        ...
        # 4️⃣ Test with full context (names + IDs)
        test_valid_chain(chain, game["start_name"], game["end_name"], game["start_id"], game["end_id"])
    else:
